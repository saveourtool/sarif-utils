...
fun Project.configurePublishing() {
    ...
    // Publication with name `sarif-utils` is for the default artifact.
    // `configureNexusPublishing` adds sonatype publication tasks inside `afterEvaluate`.
    afterEvaluate {
        val publicationsFromMainHost = listOf("jvm", "linuxX64", "kotlinMultiplatform", "metadata")
    ...
    configure<PublishingExtension> {
        ...
        publications.withType<MavenPublication>().forEach { publication ->
            ...
            publication.pom {
                ...
                url.set("https://github.com/saveourtool/sarif-utils")
                licenses {
                    license {
                ...
                developers {
                    developer {
                        id.set("kgevorkyan")
                        name.set("Kirill Gevorkyan")
                        email.set("26010098+kgevorkyan@users.noreply.github.com")
                    }
                }
                scm {
                    url.set("https://github.com/saveourtool/sarif-utils")
                    connection.set("scm:git:git://github.com/saveourtool/sarif-utils.git")
                }
            }
        }
    }
